...
max_iters = 5000
eval_interval = 300 # number of steps between evaluations
learning_rate = 1e-3
device = 'cuda' if torch.cuda.is_available() else 'cpu'
eval_iters = 200 # number of batches to evaluate for loss estimates
...
class Head(nn.Module):
    ...
    def __init__(self,head_size):
        super().__init__()
        self.key = nn.Linear(n_embd,head_size,bias = False) # 作用域C的最后一个维度，将其线性变换为head_size
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        x = tok_emb + pos_emb # (B,T,C) ,将token embedding和position embedding相加，得到输入x
        x = self.blocks(x)
        x = self.ln_f(x)
